...
(function(){
  function init(){
    ...
    var REGIONS=[['all','Tous les itinéraires'],['europe','Europe'],['africa','Afrique'],['asia','Asie'],['us','États-Unis'],['transatlantic','Transatlantique']];
    var SORTS=[['soon','Au plus tôt'],['priceasc','Prix : croissant'],['pricedesc','Prix : décroissant']];
    var CATORDER=['Very light','Turboprops','Light','Midsize','Super midsize','Heavy','Long-range'];
    var CATLABEL={'Very light':'Très léger','Turboprops':'Turbopropulseurs','Light':'Léger','Midsize':'Midsize','Super midsize':'Super-midsize','Heavy':'Lourd','Long-range':'Long-courrier'};
    ...
    function card(l){var pr=ELp(l);
      var msg=encodeURIComponent("Bonjour Andreea, je souhaiterais réserver le vol à vide "+l.from+" vers "+l.to+" le "+l.date+" (à partir de "+l.price+"). Est-il toujours disponible ?");
      return '<a class="empty-card" href="https://wa.me/971503372980?text='+msg+'" target="_blank" rel="noopener">'
       +'<div class="empty-card-top"><div class="empty-route">'+l.from+' → '+l.to+'</div><div class="empty-badge">Vol à vide</div></div>'
       +'<div class="empty-meta"><div class="empty-date">'+l.date+'</div><div class="empty-time">'+l.aircraft+'</div></div>'
       +'<div class="empty-card-bottom"><div><div class="empty-price">À partir de '+l.price+'</div><div class="empty-aircraft">'+catlab(l.cat)+' · '+l.seats+' sièges</div></div><div class="empty-cta">'+WA+' Demander sur WhatsApp</div></div>'
       +'<div class="empty-prob"><div class="empty-prob-label">Probabilité de confirmation</div><div class="empty-prob-bar-wrap"><div class="empty-prob-bar" style="width:'+pr+'%"></div></div><div class="empty-prob-pct">'+pr+'%</div></div>'
       +'</a>';
    }
    ...
    function labels(){
      ...
      var r=REGIONS.filter(function(x){return x[0]===state.region;})[0];L('region',state.region==='all'?'Région':r[1],state.region!=='all');
      var n=catsActive().length;L('jets',n?'Appareil ('+n+')':'Appareil',n>0);
      L('when',state.month==='all'?'Quand':(MN[state.month%100]+' '+Math.floor(state.month/100)),state.month!=='all');
      var t='Prix';if(state.min!=null||state.max!=null)t=(state.min!=null?'€'+state.min:'€0')+'–'+(state.max!=null?'€'+state.max:'∞');L('price',t,state.min!=null||state.max!=null);
      var so=SORTS.filter(function(x){return x[0]===state.sort;})[0];L('sort',state.sort==='soon'?'Trier':so[1],state.sort!=='soon');
    }
    function render(){
      ...
      var sh=document.getElementById('elShown');if(sh)sh.textContent=list.length+(list.length===1?' vol à vide':' vols à vide');
    ...
    buildPanel('ddWhen',[['all','Toutes les dates']].concat(months().map(function(m){return [String(m[0]),m[1],m[2]];})),function(){return String(state.month);},function(v){state.month=v==='all'?'all':parseInt(v,10);});
