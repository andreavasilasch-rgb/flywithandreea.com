...
(function(){
  function init(){
    ...
    var REGIONS=[['all','Todas las rutas'],['europe','Europa'],['africa','África'],['asia','Asia'],['us','Estados Unidos'],['transatlantic','Transatlántico']];
    var SORTS=[['soon','Más próximos'],['priceasc','Precio: ascendente'],['pricedesc','Precio: descendente']];
    var CATORDER=['Very light','Turboprops','Light','Midsize','Super midsize','Heavy','Long-range'];
    var CATLABEL={'Very light':'Very light','Turboprops':'Turbohélice','Light':'Light','Midsize':'Midsize','Super midsize':'Super-midsize','Heavy':'Pesado','Long-range':'Largo alcance'};
    ...
    function card(l){var pr=ELp(l);
      var msg=encodeURIComponent("Hola Andreea, me gustaría reservar el empty leg "+l.from+" a "+l.to+" el "+l.date+" (desde "+l.price+"). ¿Sigue disponible?");
      return '<a class="empty-card" href="https://wa.me/971503372980?text='+msg+'" target="_blank" rel="noopener">'
       ...
       +'<div class="empty-card-bottom"><div><div class="empty-price">Desde '+l.price+'</div><div class="empty-aircraft">'+catlab(l.cat)+' · '+l.seats+' asientos</div></div><div class="empty-cta">'+WA+' Consultar por WhatsApp</div></div>'
       +'<div class="empty-prob"><div class="empty-prob-label">Probabilidad de confirmación</div><div class="empty-prob-bar-wrap"><div class="empty-prob-bar" style="width:'+pr+'%"></div></div><div class="empty-prob-pct">'+pr+'%</div></div>'
       +'</a>';
    }
    ...
    function labels(){
      ...
      var r=REGIONS.filter(function(x){return x[0]===state.region;})[0];L('region',state.region==='all'?'Región':r[1],state.region!=='all');
      var n=catsActive().length;L('jets',n?'Aeronave ('+n+')':'Aeronave',n>0);
      L('when',state.month==='all'?'Cuándo':(MN[state.month%100]+' '+Math.floor(state.month/100)),state.month!=='all');
      var t='Precio';if(state.min!=null||state.max!=null)t=(state.min!=null?'€'+state.min:'€0')+'–'+(state.max!=null?'€'+state.max:'∞');L('price',t,state.min!=null||state.max!=null);
      var so=SORTS.filter(function(x){return x[0]===state.sort;})[0];L('sort',state.sort==='soon'?'Ordenar':so[1],state.sort!=='soon');
    }
    function render(){
    ...
    buildPanel('ddWhen',[['all','Todas las fechas']].concat(months().map(function(m){return [String(m[0]),m[1],m[2]];})),function(){return String(state.month);},function(v){state.month=v==='all'?'all':parseInt(v,10);});
