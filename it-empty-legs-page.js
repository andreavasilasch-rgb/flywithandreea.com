...
(function(){
  function init(){
    ...
    var REGIONS=[['all','Tutte le rotte'],['europe','Europa'],['africa','Africa'],['asia','Asia'],['us','Stati Uniti'],['transatlantic','Transatlantico']];
    var SORTS=[['soon','Più imminenti'],['priceasc','Prezzo: crescente'],['pricedesc','Prezzo: decrescente']];
    var CATORDER=['Very light','Turboprops','Light','Midsize','Super midsize','Heavy','Long-range'];
    var CATLABEL={'Very light':'Very light','Turboprops':'Turboelica','Light':'Light','Midsize':'Midsize','Super midsize':'Super-midsize','Heavy':'Pesante','Long-range':'Long range'};
    ...
    function card(l){var pr=ELp(l);
      var msg=encodeURIComponent("Ciao Andreea, vorrei prenotare il volo empty leg "+l.from+" verso "+l.to+" il "+l.date+" (da "+l.price+"). È ancora disponibile?");
      return '<a class="empty-card" href="https://wa.me/971503372980?text='+msg+'" target="_blank" rel="noopener">'
       ...
       +'<div class="empty-card-bottom"><div><div class="empty-price">Da '+l.price+'</div><div class="empty-aircraft">'+catlab(l.cat)+' · '+l.seats+' posti</div></div><div class="empty-cta">'+WA+' Chiedi su WhatsApp</div></div>'
       +'<div class="empty-prob"><div class="empty-prob-label">Probabilità di conferma</div><div class="empty-prob-bar-wrap"><div class="empty-prob-bar" style="width:'+pr+'%"></div></div><div class="empty-prob-pct">'+pr+'%</div></div>'
       +'</a>';
    }
    ...
    function labels(){
      ...
      var r=REGIONS.filter(function(x){return x[0]===state.region;})[0];L('region',state.region==='all'?'Regione':r[1],state.region!=='all');
      var n=catsActive().length;L('jets',n?'Aeromobile ('+n+')':'Aeromobile',n>0);
      L('when',state.month==='all'?'Quando':(MN[state.month%100]+' '+Math.floor(state.month/100)),state.month!=='all');
      var t='Prezzo';if(state.min!=null||state.max!=null)t=(state.min!=null?'€'+state.min:'€0')+'–'+(state.max!=null?'€'+state.max:'∞');L('price',t,state.min!=null||state.max!=null);
      var so=SORTS.filter(function(x){return x[0]===state.sort;})[0];L('sort',state.sort==='soon'?'Ordina':so[1],state.sort!=='soon');
    }
    function render(){
      ...
      var sh=document.getElementById('elShown');if(sh)sh.textContent=list.length+(list.length===1?' volo empty leg':' voli empty leg');
    ...
    buildPanel('ddWhen',[['all','Tutte le date']].concat(months().map(function(m){return [String(m[0]),m[1],m[2]];})),function(){return String(state.month);},function(v){state.month=v==='all'?'all':parseInt(v,10);});
