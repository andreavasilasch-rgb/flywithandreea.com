...
(function(){
  ...
  var EXT = ['png','jpg','jpeg','webp','avif'];  // her extras use .jpeg and .avif too
  ...
  // Service cards carry an exact /extras/card-<service>.<ext> path. If one of
  // those files is ever missing the panel would sit empty, so each card is
  // probed once and falls back to the matching aircraft render.
  var CARD_FALLBACK = {
    'card-private-jet-charter':'/hero-photo.webp',
    'card-executive-airliners':'/bombardier-challenger-850.png',
    'card-vip-airliners':'/embraer-lineage-1000.png',
    'card-group-charter':'/bombardier-global-7500.png',
    'card-pet-friendly-flights':'/extras/pets-cabin.jpg',
    'card-empty-legs':'/embraer-phenom-300.png',
    'card-private-jet-packages':'/extras/special-occasions.jpg',
    'card-air-ambulance':'/extras/medical.jpg'
  };
  function cardFallback(){
    document.querySelectorAll('.svccard-img').forEach(function(el){
      var m=(el.style.backgroundImage||'').match(/url\(["']?([^"')]+)/);
      if(!m) return;
      var url=m[1], key=(url.split('/').pop()||'').replace(/\.[a-z0-9]+$/i,'');
      var alt=CARD_FALLBACK[key];
      if(!alt) return;
      var probe=new Image();
  ...
  function init(){ cardFallback(); buildGallery(); document.querySelectorAll('[data-fwa-img]').forEach(resolve); }
