...
(function(){
  function init(){
    ...
    var REGIONS=[['all','All routes'],['europe','Europe'],['africa','Africa'],['asia','Asia'],['us','United States'],['transatlantic','Transatlantic']];
    var SORTS=[['soon','Soonest'],['priceasc','Price: low to high'],['pricedesc','Price: high to low']];
    ...
    buildPanel('ddWhen',[['all','All dates']].concat(months().map(function(m){return [String(m[0]),m[1],m[2]];})),function(){return String(state.month);},function(v){state.month=v==='all'?'all':parseInt(v,10);});
    ...
    if(jp){var counts={};legs.forEach(function(l){counts[l.cat]=(counts[l.cat]||0)+1;});
      ...
      jp.innerHTML=keys.map(function(k){return '<label class="el-check"><input type="checkbox" data-j="'+k.replace(/"/g,'')+'"><span class="el-box"></span><span class="el-lab">'+k+'</span><span class="el-c">'+counts[k]+'</span></label>';}).join('');
